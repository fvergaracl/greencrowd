    ...
    if (campaign.isJoined) {
      ...
      try {
        ...
        if (response.ok) {
          setCampaigns((prev) =>
            prev.map((c) =>
              c.id === campaign.id ? { ...c, isJoined: true } : c
            )
          );
  ...
  return (
    <div className=" flex flex-col items-center bg-gray-50 p-4">
      ...
      <button
        onClick={() => {
          logEvent(
            "CLICK_MY_ACTIVITY_BUTTON_FROM_CAMPAIGNS_SCREEN",
            "User clicked on My Activity button"
          );
          router.push("/dashboard/task/MyTaskActivity");
        }}
        className="mb-6 px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm hover:bg-indigo-700 transition"
        data-cy="my-activity-button"
      >
        📋 {t("View My Activity")}
      </button>
